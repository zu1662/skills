...
`SKILL.md` 只放通用业务说明。`argument-hint`、`disable-model-invocation`、`user-invocable`、`allowed-tools`、`context` 等工具专属字段,维护在仓库根目录下对应工具配置中,例如 `.claude/commands/<name>.md`。OpenCode command 会由同一份 Markdown command 生成到 `~/.config/opencode/opencode.json`。
